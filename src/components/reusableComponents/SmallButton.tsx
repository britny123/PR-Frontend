...
export default function SmallButton({text ,onClick,}: SmallButtonProps) {
  return (
    <button
      onClick={onClick}
      className=" w-32 h-5 items-center justify-center flex rounded-full border border-blue text-blue font-semibold transition-all duration-200 hover:-translate-y-1">
      {text}
    </button>
  );
}